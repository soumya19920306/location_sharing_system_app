class SharedLocationRegister < ApplicationRecord
  ...
  def self.get_all_locations_shared_with_me(user_id)
    shared_location_objs = SharedLocationRegister.includes(:shared_location_user_mappings)
      .joins(:shared_location_user_mappings).where("shared_location_user_mappings.shared_user_id = #{ user_id }" )
  ...
  def self.get_all_my_shared_locations(user_id, is_only_public_sharings = false)
    ...
    shared_location_objs = SharedLocationRegister.includes(:shared_location_user_mappings)
      .references(:shared_location_user_mappings).where(where_str)
	  
    respond_obj = []
    shared_location_objs.each do |loc|
    _inner_hash = loc.as_json
    shared_user_id = loc.shared_location_user_mappings.pluck(:shared_user_id)
      if shared_user_id.present? && shared_user_id.length == 1 && shared_user_id.include?(PUBLIC_USER_ID)
